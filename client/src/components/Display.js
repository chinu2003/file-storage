  ...
  const getdata = async () => {
    ...
    if (dataArray.length > 0) {
      ...
      const images = dataArray.map((item, i) => (
        <a href={extractIPFSUrl(item)} key={i} target="_blank" rel="noopener noreferrer">
          <img src={extractIPFSUrl(item)} alt="IPFS Image" className="image-list" crossOrigin="anonymous" />
        </a>
      ));
